...
bool Transcriber::GenerateKaraoke(const char* inputPath, const char* outputDir)
{
    ...
    // converting to a wav file if needed
    bool isInputWav = StringUtils::GetFileExtension(inputPath) == "wav";
    if(!isInputWav){
        std::string wavFile = StringUtils::ExtractFileName(inputPath) + "-temp.wav";
        VideoUtils::Generate16KHzWav(inputPath, wavFile);
        inputPath = wavFile.c_str();   
    }

    // WERID BUG: value of inputPath gets corrupted after writing to a file for some reasons
    std::string inputFile = inputPath; 
    ...
    if (result && std::remove(outputScript.c_str()) == 0)
    {
        if(!isInputWav) 
        {
            fprintf(stderr, "%s: Removing generated wav file : '%s'\n", __func__, inputFile.c_str());
            std::remove(inputFile.c_str()); // removing if the input file was generated by program
        }
